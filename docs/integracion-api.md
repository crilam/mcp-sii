...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **Apagado por defecto: `RESPALDO_XML_TERCER_NIVEL`.** La combinación `tipo_dte`+`folio_desde`/`folio_hasta` y `tipo_dte`+`contraparte_rut` —las que usa este tercer nivel— **no están verificadas end-to-end contra el SII real** (a diferencia de fecha+folio y fecha+contraparte, que sí lo están). Si el CGI ignorara `TPO_DOC` en presencia de estos otros filtros (o viceversa), cada folio terminaría "dato roto" en la bisección y un solo día lleno quemaría todo `max_tramos` en llamadas inútiles contra un portal que bloquea por patrón de uso. Por eso el tercer nivel queda **desactivado por defecto**: un día con `tipo_dte` que excede el tope registra una limitación explicando que está apagado y cómo prenderlo, en vez de intentarlo a ciegas. La variable de entorno `RESPALDO_XML_TERCER_NIVEL` en `1` o `true` (sin importar mayúsculas) lo activa — hacelo recién después de correr `src/scripts/verificarRespaldoXml.ts` contra un día real que exceda el tope con `tipo_dte`+folio y `tipo_dte`+contraparte, y confirmar que el CGI respeta la combinación.
