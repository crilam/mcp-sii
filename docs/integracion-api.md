...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **`limitaciones`: qué sub-rango NO se pudo bajar, y por qué.** Es una lista, **siempre presente** (vacía cuando no hubo ningún corte), de `{fecha_desde, fecha_hasta, motivo}` más, opcionalmente, `tipo_dte`, `contraparte_rut`, `folio_desde` y `folio_hasta` cuando el motivo sale del tercer nivel: esos campos son EXACTAMENTE el filtro que hay que repetir para pedir de nuevo ese sub-rango, sin tener que parsear el texto de `motivo`. Cada elemento sale de uno de tres fallos del troceo —un día suelto con más de 20 documentos que ya no se puede partir por fecha, un rango (o un folio único) que necesitaría más tramos que `max_tramos`, o un folio que por sí solo excede el tope y es un dato roto— y el `motivo` dice qué hacer: pedir ese sub-rango con `tipo_dte`, acotar el rango, o pedirlo por folio/contraparte. Las limitaciones **contiguas con el mismo `motivo`** llegan ya fusionadas en un solo elemento con el rango unido, para no obligar al consumidor a pedir por separado varios sub-rangos vecinos cuando uno solo alcanza (la fusión exige, además del `motivo` textual, que `tipo_dte`/`folio_desde`/`folio_hasta`/`contraparte_rut` también coincidan, para no perder alguno de estos campos estructurados al unir). **OJO**: el motivo de "día lleno" y el del tercer nivel embeben la fecha (y el folio o la contraparte, en su caso) en el propio texto, así que dos sub-rangos vecinos con ese motivo casi nunca fusionan —el texto de cada uno ya difiere aunque la causa sea la misma—; en la práctica, la fusión aplica sobre todo al motivo de tope de `max_tramos`, que es genérico. **Una excepción sin campo estructurado:** cuando el tercer nivel de `recibidos` agota `max_tramos` entre un emisor y el siguiente, la limitación de "quedaron N emisores sin procesar" **no lleva `contraparte_rut`** —ningún RUT individual representa a los N que quedaron— y en cambio nombra los RUT pendientes en texto libre dentro de `motivo` (los primeros 10, con un resumen si sobran).
