...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **Apagado por defecto: `RESPALDO_XML_TERCER_NIVEL`.** La combinación `tipo_dte`+`folio_desde`/`folio_hasta` (eje de `emitidos`) **ya se verificó** end-to-end contra el SII real (ver el párrafo de arriba); la combinación `tipo_dte`+`contraparte_rut` (eje de `recibidos`) **sigue sin verificarse**. `RESPALDO_XML_TERCER_NIVEL` es un interruptor único que habilita **los dos ejes a la vez**, sin granularidad: no hay forma de prender folio (verificado) y dejar contraparte (no verificado) apagado. Prenderlo en un ambiente que reciba consultas de `recibidos` habilita también el eje no medido. Si el CGI ignorara `TPO_DOC` en presencia de `RUT_RECP`, cada folio terminaría "dato roto" en la bisección y un solo día lleno quemaría todo `max_tramos` en llamadas inútiles contra un portal que bloquea por patrón de uso. Por eso, aunque el eje de folio ya esté verificado, el tercer nivel completo queda **desactivado por defecto**: prenderlo abre un eje más de llamadas contra ese portal, así que activarlo en producción es una decisión de despliegue que se toma a conciencia, no un default. Un día con `tipo_dte` que excede el tope registra una limitación explicando que está apagado y cómo prenderlo, en vez de intentarlo a ciegas. La variable de entorno `RESPALDO_XML_TERCER_NIVEL` en `1` o `true` (sin importar mayúsculas) lo activa — antes de prenderlo en un ambiente que atienda `recibidos`, corré `src/scripts/verificarRespaldoXml.ts` contra un día real que exceda el tope con `tipo_dte`+contraparte y confirmá que el CGI respeta la combinación, igual que ya se hizo para folio. La medición del 2026-09-11 encontró además que el propio CGI de listado de `recibidos` (`mipeAdminDocsRcp.cgi`) falló con la página de error genérica del portal en varias sesiones seguidas, independiente del volumen de la empresa consultada; ver la evaluación de usar el RCV como fuente alternativa del eje de contraparte en `docs/superpowers/specs/2026-09-11-tercer-nivel-recibidos-fuente-rcv.md` (sin implementar, con preguntas abiertas).
  ...
  **`causa`**: discriminador ESTRUCTURADO, siempre presente en la respuesta. El criterio para cada valor es qué acción induce en quien lo lee, no sólo "es un corte distinto":
  ...
  | Tercer nivel apagado (`tipo_dte`+folio/contraparte, sin `RESPALDO_XML_TERCER_NIVEL`) | OTRA | Sí | No | No | Prender el flag recién tras verificarlo, o acotar de otra forma |
