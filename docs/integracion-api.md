...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **`limitaciones`: qué sub-rango NO se pudo bajar, y por qué.** Es una lista, **siempre presente** (vacía cuando no hubo ningún corte), de `{fecha_desde, fecha_hasta, motivo}` más, opcionalmente, `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde` y `folio_hasta` cuando el motivo sale del tercer nivel: en general esos campos son el filtro exacto que hay que repetir para pedir de nuevo ese sub-rango, sin tener que parsear el texto de `motivo` —**salvo la excepción marcada en la tabla**, donde `folio_desde`/`folio_hasta` son un rango envolvente, no exacto—. `razon_social` viaja junto con `tipo_dte` (ambos "Sí" o ambos "No" en la tabla): es el mismo filtro de listado, así de acotado o no como el llamador lo haya pedido. Qué campos trae cada motivo:

  | Motivo (resumen) | `tipo_dte` / `razon_social` | `contraparte_rut` | `folio_desde`/`folio_hasta` | Qué hacer |
  |---|---|---|---|---|
  | Día suelto excede el tope, sin `tipo_dte` puesto | No | No | No | Pedir ese día con `tipo_dte` |
  | Tercer nivel apagado (`tipo_dte`+folio/contraparte, sin `RESPALDO_XML_TERCER_NIVEL`) | Sí | No | No | Prender el flag recién tras verificarlo, o acotar de otra forma |
  | Rango (o día lleno) agota `max_tramos`, sin tercer nivel | No | No | No | Acortar el rango o subir `max_tramos` |
  | Folio único que excede por sí solo el tope (dato roto) | Sí | Si se pidió | Sí (`folio_desde == folio_hasta`) | No se puede afinar más; es un dato a revisar, no un sub-rango |
  | Grupo de folios agota `max_tramos` (tercer nivel) | Sí | Si aplica (`emitidos` con contraparte, o `recibidos`) | Sí, pero **ENVOLVENTE**: `Math.min`/`Math.max` de los folios pendientes, no necesariamente contiguo — puede incluir folios ya bajados o de otro tipo si quedaron huecos entre grupos | Subir `max_tramos`, o repetir el rango (seguro, pero baja de más) |
  | El listado no devolvió folios/emisores del día+tipo (o del rango pedido) | Sí | Si se pidió | Si se pidió | El listado y la descarga no cuentan igual; no hay con qué trocear más fino |
  | El listado agota `max_tramos` a mitad de la paginación | Sí | No | No | Subir `max_tramos` |
  | `recibidos`: quedaron emisores sin procesar (presupuesto agotado ENTRE emisores) | Sí | **No** | No | Subir `max_tramos`, o acotar con `contraparte_rut` — los RUT pendientes (hasta 10, con resumen si sobran) van en el texto de `motivo`, no en un campo estructurado: ningún RUT individual representa a los N que quedaron |
  ...
  **Documentos con un nombre de tipo que el scraper no reconoce.** El listado HTML nombra el tipo de documento con texto libre (`"Factura Electronica"`, `"Guia de Despacho Electronica"`, ...) y un mapa interno lo traduce a `tipo_dte`; si el portal usa una variante de nombre que el mapa no tiene, ese documento no se pierde —se sigue troceando junto con los del tipo pedido—, pero cualquier `motivo` de ese día+tipo agrega una frase avisando que el mapa puede ser la causa real de un resultado inesperado, en vez de asumir que es el portal.
  ...
  Las limitaciones **contiguas con el mismo `motivo`** llegan ya fusionadas en un solo elemento con el rango unido, para no obligar al consumidor a pedir por separado varios sub-rangos vecinos cuando uno solo alcanza (la fusión exige, además del `motivo` textual, que `tipo_dte`/`folio_desde`/`folio_hasta`/`contraparte_rut`/`razon_social` también coincidan, para no perder alguno de estos campos estructurados al unir). En la práctica aplica sobre todo al motivo genérico de tope de `max_tramos`: los demás motivos embeben la fecha (y el folio o la contraparte) en el propio texto, así que casi nunca coinciden entre sub-rangos vecinos.
