...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **`limitaciones`: qué sub-rango NO se pudo bajar, y por qué.** Es una lista, **siempre presente** (vacía cuando no hubo ningún corte), de `{fecha_desde, fecha_hasta, motivo}` más, opcionalmente, `tipo_dte`, `contraparte_rut`, `folio_desde` y `folio_hasta` cuando el motivo sale del tercer nivel: esos campos son EXACTAMENTE el filtro que hay que repetir para pedir de nuevo ese sub-rango, sin tener que parsear el texto de `motivo`. Qué campos trae cada motivo:
  ...
  | Grupo de folios agota `max_tramos` (tercer nivel) | Sí | Si aplica (`emitidos` con contraparte, o `recibidos`) | Sí | Subir `max_tramos` o acotar el rango de folios |
