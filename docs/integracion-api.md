...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **Filtrar no es sólo comodidad: es lo que baja el costo de la llamada.** Cada filtro recorta el conjunto, y con el tope de 20 documentos por descarga eso significa menos tramos, menos llamadas al portal y menos latencia. Todos se verificaron contra el SII:
  ...
  | `folio_desde` / `folio_hasta` | Rango de folios. `folio_desde` solo = ese folio exacto. **En `origen=recibidos` es ese mismo rango dentro de CADA emisor**, no un rango global: los folios de distintos emisores colisionan (cada uno numera el suyo), así que "folio 10 a 20" filtra los folios 10 a 20 de CADA contraparte del período, no un tramo único de la numeración. | folio 739 → 1 documento |
  ...
  Los filtros **combinados** en la misma llamada (fecha + folio, fecha + contraparte) están verificados contra el SII: las dos llamadas al portal (`lista_documentos.cgi` y `download.cgi`) siempre llevan `FEC_DESDE`/`FEC_HASTA` —son obligatorios— junto con el resto, así que probar `folio_desde` o `contraparte_rut` de forma aislada ya los probó combinados con fecha. La combinación de `tipo_dte` con `folio_desde`/`folio_hasta` —una de las dos que usa el tercer nivel de troceo, más abajo— **sí se verificó end-to-end contra el SII real**: una consulta de un día con un mes cargado, con `tipo_dte` fijo y un rango de folio acotado, se midió dos veces con anchos de rango distintos (del orden de mil y de cinco mil) y en las dos el CGI respetó ambos filtros a la vez, entregando documentos reales (6 y 18 respectivamente) verificados uno por uno. La combinación de `tipo_dte` con `contraparte_rut` —la otra mitad del tercer nivel, la que usan los `recibidos`— **no se probó end-to-end contra el SII real**; se infiere de que el CGI trata los cinco campos (`FEC_DESDE`, `FEC_HASTA`, `TPO_DOC`, `FOLIO`/`FOLIOHASTA`, `RUT_RECP`) como parámetros de formulario independientes, cada uno ya verificado por separado en combinación con la fecha.

  **Ficha de la medición (lugar canónico):** corrida en vivo el 2026-09-11, contra una empresa real con un mes cargado, un día concreto de ese mes (sin identificar cuál) y `tipo_dte=33`. Se hicieron dos consultas con `folio_desde`/`folio_hasta` fijos, con anchos de rango de folio distintos —del orden de mil y de cinco mil—, y las dos devolvieron documentos reales del tipo pedido dentro del rango pedido (6 y 18 respectivamente, verificados uno por uno). No se probó `contraparte_rut` en la misma sesión: ese eje sigue sin medición en vivo.
  ...
  **No hay `iecv` acá.** Los libros de compras y ventas existen en el portal pero se bajan por otro CGI (`respaldoLibrosXml.cgi`), **por código de libro y no por rango de fechas**, y se filtran por `TPO_LIBRO` (VENTA/COMPRA) en vez de por contraparte o folio. No entra en el contrato de esta ruta; sería una ruta aparte.
  ...
  Hay más de un tramo porque **el SII no entrega más de 20 documentos por descarga**, y el tope lo aplica el servidor: cuando el rango lo excede se parte al medio y cada mitad se pide aparte. Cada tramo extra es otra llamada al portal dentro de la misma request, y por eso `max_tramos` la acota.

  **El troceo por fecha tiene un piso de ancho de 7 días, y no depende sólo del aviso del portal.** Medido en vivo (una empresa con un mes cargado): un rango de fecha ANCHO no siempre dispara el aviso de "más de 20 documentos" cuando lo excede — un mes completo sin filtro volvió con pocos documentos y `ok:true`, sin bisectar nunca, mientras un solo día de ese mismo mes sí avisó que él solo ya excedía. El portal **trunca en silencio** en rangos anchos en vez de avisar, y ese truncamiento es indistinguible de un mes con pocos documentos. Por eso, antes de pedir cualquier rango de fecha, el servicio lo compara contra este piso: si es más ancho que 7 días, lo **parte sin pedirlo** —en tramos lo más PAREJOS posible dentro del piso, no bisectando a la mitad— y recursa sobre ellos, sin gastar una llamada en el rango ancho, que medido no avisa nada igual. Recién cuando el rango ya está dentro de esos 7 días se pide de verdad, y ahí la bisección por el aviso del portal (el párrafo de arriba) sigue funcionando igual que siempre, porque dentro de ese ancho el aviso SÍ es confiable (medido).

  **Este piso es el costo nuevo que compite por `max_tramos`, y por eso el default (10) no sube.** Un mes tranquilo (28 a 31 días, sin ningún día que exceda) pasa de costar 1 descarga a costar entre 4 y 5 —una por cada tramo parejo de ≤7 días—, y un mes con días que SÍ exceden el tope puede agotar el presupuesto por defecto antes de cubrir el rango entero (queda como una limitación `PRESUPUESTO_TRAMOS` con el rango exacto que faltó, no como un error). Subir `max_tramos` no es la respuesta: con la pausa de ritmo entre tramos, unos 16 tramos ya rondan los ~35 s que el ERP le da a esta llamada, así que un default más alto no produce respaldos más completos, produce TIMEOUTS —y un timeout es estrictamente peor que un parcial, porque el parcial dice con precisión qué rango faltó y el timeout no deja nada—. Quien de verdad necesite más lo pide explícitamente (hasta `MAX_TRAMOS_ABSOLUTO`) sabiendo que cambia ese riesgo.

  **El parcial no es la excepción: en una empresa con facturación diaria es la norma.** Con volumen constante (documentos todos los días, de sobra para que cualquier tramo de más de un día exceda el tope), el presupuesto por defecto no cubre el mes: baja los primeros días individuales hasta agotarse y el resto queda como UNA limitación `PRESUPUESTO_TRAMOS`, con el rango exacto que faltó (no un mes entero ni un rango aproximado). No es un defecto del piso —es el mismo techo de `max_tramos` que ya existía antes de él—, pero conviene esperarlo como el caso típico de una PyME activa, no como el raro.

  **Tercer nivel de troceo: cuando ni la fecha ni el tipo alcanzan.** El troceo por fecha llega hasta el DÍA; si con `tipo_dte` puesto un día suelto TODAVÍA excede el tope, hace falta un eje más fino, y ese eje depende de quién numera los documentos:

  - **`emitidos`**: el **folio** ordena, porque la empresa es la emisora y sus folios de un mismo tipo no se repiten. Se listan los folios de ese día y tipo (el mismo listado de `list-dte-emitidos`), se ordenan y se agrupan de a lo sumo 20; cada grupo se pide con `folio_desde`/`folio_hasta`. Si un grupo TODAVÍA excede —el listado y la descarga no cuentan igual, ver más abajo— se bisecta por folio hasta llegar a uno solo.
  - **`recibidos`**: los folios de distintos emisores **colisionan** (cada uno numera el suyo), así que el eje es la **contraparte**. Se listan los emisores de ese día y tipo (el listado de `list-dte-recibidos`) y se pide una descarga por emisor; sólo si UN emisor por sí solo excede el tope se bisecta también por folio, dentro de ese emisor.

  Este tercer nivel **sólo se activa con `tipo_dte` puesto**. Sin él, un día que excede el tope sigue devolviendo la limitación de siempre («pedí ese día con `tipo_dte`»): ese primer corte lo maneja el consumidor, no el servicio. Cada llamada del tercer nivel —listados incluidos— cuenta contra `max_tramos` y lleva la misma pausa de ritmo que las demás.

  **Apagado por defecto, una variable POR EJE: `RESPALDO_XML_TERCER_NIVEL_FOLIO` y `RESPALDO_XML_TERCER_NIVEL_CONTRAPARTE`.** La combinación `tipo_dte`+`folio_desde`/`folio_hasta` (eje de `emitidos`) **ya se verificó** end-to-end contra el SII real (ver el párrafo de arriba); la combinación `tipo_dte`+`contraparte_rut` (eje de `recibidos`) **sigue sin verificarse**. Como los dos ejes NO tienen el mismo respaldo de evidencia, cada uno tiene su propia variable — nunca una sola para los dos —: `RESPALDO_XML_TERCER_NIVEL_FOLIO` prende **sólo** el eje verificado (folio, emitidos) y `RESPALDO_XML_TERCER_NIVEL_CONTRAPARTE` prende **sólo** el eje no verificado (contraparte, recibidos), sin que prender uno habilite el otro. Si el CGI ignorara `TPO_DOC` en presencia de `RUT_RECP`, cada folio terminaría "dato roto" en la bisección y un solo día lleno quemaría todo `max_tramos` en llamadas inútiles contra un portal que bloquea por patrón de uso — ese riesgo es exclusivo del eje de contraparte y ya no se hereda al activar el eje de folio. Las dos variables quedan **desactivadas por defecto**: prender cualquiera abre un eje más de llamadas contra ese portal, así que activarla en producción es una decisión de despliegue que se toma a conciencia, no un default. Un día con `tipo_dte` que excede el tope registra una limitación explicando cuál de las dos variables corresponde y cómo prenderla, en vez de intentarlo a ciegas. Cada variable en `1` o `true` (sin importar mayúsculas) activa su eje — antes de prender `RESPALDO_XML_TERCER_NIVEL_CONTRAPARTE` en un ambiente que atienda `recibidos`, corré `src/scripts/verificarRespaldoXml.ts` contra un día real que exceda el tope con `tipo_dte`+contraparte y confirmá que el CGI respeta la combinación, igual que ya se hizo para folio.

  **Compatibilidad con el interruptor único original.** `RESPALDO_XML_TERCER_NIVEL` sigue existiendo y, si está puesto, prende **los dos ejes a la vez** — igual que siempre hizo —, para no romper a quien ya lo usa en algún ambiente. Las variables por eje, cuando están presentes, tienen **precedencia** sobre él (incluso si valen `'0'`), lo que permite por ejemplo dejar el heredado prendido y apagar explícitamente sólo el eje no verificado.

  La medición del 2026-09-11 encontró además que el propio CGI de listado de `recibidos` (`mipeAdminDocsRcp.cgi`) falló con la página de error genérica del portal en varias sesiones seguidas, independiente del volumen de la empresa consultada; ver la evaluación de usar el RCV como fuente alternativa del eje de contraparte en `docs/superpowers/specs/2026-09-11-tercer-nivel-recibidos-fuente-rcv.md` (sin implementar, con preguntas abiertas).

  **`limitaciones`: qué sub-rango NO se pudo bajar, y por qué.** Es una lista, **siempre presente** (vacía cuando no hubo ningún corte), de `{fecha_desde, fecha_hasta, motivo, causa}` más, opcionalmente, `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde` y `folio_hasta` cuando el motivo sale del tercer nivel: en general esos campos son el filtro exacto que hay que repetir para pedir de nuevo ese sub-rango, sin tener que parsear el texto de `motivo` —**salvo la excepción marcada en la tabla**, donde `folio_desde`/`folio_hasta` son un rango envolvente, no exacto—. `razon_social` viaja junto con `tipo_dte` (ambos "Sí" o ambos "No" en la tabla): es el mismo filtro de listado, así de acotado o no como el llamador lo haya pedido. Qué campos trae cada motivo:

  **`causa`**: discriminador ESTRUCTURADO, siempre presente en la respuesta. El criterio para cada valor es qué acción induce en quien lo lee, no sólo "es un corte distinto":

  - `"PRESUPUESTO_TRAMOS"`: subir `max_tramos` (o acotar el rango) ARREGLA el corte. Las filas de la tabla marcadas así.
  - `"SII_NO_DISPONIBLE"`: el portal devolvió su página de error genérica varias veces seguidas — un fallo TRANSITORIO (mismo significado que el error `SII_NO_DISPONIBLE` de otras rutas de este servicio). **Nunca subir `max_tramos` acá**: reintentar con más presupuesto contra un portal caído es el barrido de llamadas que este mismo servicio documenta como la causa real de que el SII bloquee el portal. Lo único que corresponde es esperar y reintentar más tarde.
  - `"OTRA"`: clasificado, y ninguna acción genérica (ni subir `max_tramos` ni esperar) arregla el corte por igual — ver `motivo` de cada uno para el detalle accionable. Los 26 sitios de `mipymeHttp.ts` que arman una limitación clasifican explícitamente con uno de estos tres valores; NO es el resultado por defecto de "no se supo clasificar".
  - `"SIN_CLASIFICAR"`: distinto de `"OTRA"` a propósito. Significa que el scraper no puso ningún valor en `causa` para ese corte — hoy no debería pasar (los 26 sitios están clasificados), y si aparece es una señal de que se agregó un corte nuevo sin clasificar, no que se investigó y no aplica ninguna acción particular. La ruta pone este valor cuando `causa` falta, en vez de `"OTRA"`, para no mentirle al consumidor diciéndole "es otra cosa" cuando en realidad no se sabe qué es.

  Existe para no tener que parsear `motivo` en prosa cuando lo que hace falta saber es qué acción tomar. No reemplaza a `motivo`, que sigue siendo el texto completo con el detalle accionable.

  | Motivo (resumen) | `causa` | `tipo_dte` / `razon_social` | `contraparte_rut` | `folio_desde`/`folio_hasta` | Qué hacer |
  |---|---|---|---|---|---|
  | Día suelto excede el tope, sin `tipo_dte` puesto | OTRA | No | No | No | Pedir ese día con `tipo_dte` |
  | Tercer nivel apagado (`tipo_dte`+folio/contraparte, sin `RESPALDO_XML_TERCER_NIVEL_FOLIO`/`RESPALDO_XML_TERCER_NIVEL_CONTRAPARTE`) | OTRA | Sí | No | No | Prender la variable del eje que corresponde recién tras verificarlo, o acotar de otra forma |
  | Rango (o día lleno) agota `max_tramos`, sin tercer nivel | **PRESUPUESTO_TRAMOS** | No | No | No | Acortar el rango o subir `max_tramos` |
  | Folio único que excede por sí solo el tope (dato roto) | OTRA | Sí | Si se pidió | Sí (`folio_desde == folio_hasta`) | No se puede afinar más; es un dato a revisar, no un sub-rango |
  | Se acumularon demasiados folios únicos que exceden el tope por sí solos, en el mismo día+eje | OTRA | Sí | Si se pidió | Sí, **ENVOLVENTE** (igual criterio que la fila de grupo de folios de abajo) | Subir `max_tramos` NO ayuda (el problema no es de presupuesto): revisar si el SII está respetando el filtro de folio |
  | Grupo de folios agota `max_tramos` (tercer nivel) | **PRESUPUESTO_TRAMOS** | Sí | Si aplica (`emitidos` con contraparte, o `recibidos`) | Sí, pero **ENVOLVENTE**: `Math.min`/`Math.max` de los folios pendientes, no necesariamente contiguo — puede incluir folios ya bajados o de otro tipo si quedaron huecos entre grupos | Subir `max_tramos`, o repetir el rango (seguro, pero baja de más) |
  | Portal caído durante la descarga de folios del tercer nivel (varias páginas de error consecutivas) | **SII_NO_DISPONIBLE** | Sí | Si se pidió | Sí, **ENVOLVENTE** | Esperar y reintentar más tarde. NUNCA subir `max_tramos`: es el barrido que bloquea el portal |
  | El listado no devolvió folios/emisores del día+tipo (o del rango pedido) | OTRA | Sí | Si se pidió | Si se pidió | El listado y la descarga no cuentan igual; no hay con qué trocear más fino |
  | El listado agota `max_tramos` a mitad de la paginación | **PRESUPUESTO_TRAMOS** | Sí | No | No | Subir `max_tramos` |
  | `recibidos`: quedaron emisores sin procesar (presupuesto agotado ENTRE emisores) | **PRESUPUESTO_TRAMOS** | Sí | **No** | No | Subir `max_tramos`, o acotar con `contraparte_rut` — los RUT pendientes (hasta 10, con resumen si sobran) van en el texto de `motivo`, no en un campo estructurado: ningún RUT individual representa a los N que quedaron |
  | Portal caído (varios días/descargas consecutivas con la página de error genérica), en la descarga principal por rango de fecha, por emisor, o por rango de folio | **SII_NO_DISPONIBLE** | Depende del corte | Depende del corte | Depende del corte | Esperar y reintentar más tarde. NUNCA subir `max_tramos`: es el barrido que bloquea el portal |
  | El portal no contestó (una descarga puntual: tramo principal, emisor plano, o grupo de folios) | **SII_NO_DISPONIBLE** | Depende del corte | Depende del corte | Depende del corte | Esperar y reintentar más tarde. NUNCA subir `max_tramos` |
  | El listado no se pudo leer (el portal devolvió su página de error al paginar) | **SII_NO_DISPONIBLE** | Sí | No | No | Esperar y reintentar más tarde |
  | El listado tiene más páginas de las que se leen enteras | OTRA | Sí | No | No | Subir `max_tramos` NO alcanza; acotar con `contraparte_rut` o `razon_social` |

  **Documentos con un nombre de tipo que el scraper no reconoce.** El listado HTML nombra el tipo de documento con texto libre (`"Factura Electronica"`, `"Guia de Despacho Electronica"`, ...) y un mapa interno lo traduce a `tipo_dte`; si el portal usa una variante de nombre que el mapa no tiene, ese documento no se pierde —se sigue troceando junto con los del tipo pedido—, pero cualquier `motivo` de ese día+tipo agrega una frase avisando que el mapa puede ser la causa real de un resultado inesperado, en vez de asumir que es el portal.

  Las limitaciones **contiguas con el mismo texto de `motivo`** llegan ya fusionadas en un solo elemento con el rango unido, para no obligar al consumidor a pedir por separado varios sub-rangos vecinos cuando uno solo alcanza (la fusión exige, además, que `tipo_dte`/`folio_desde`/`folio_hasta`/`contraparte_rut`/`razon_social` también coincidan, para no perder alguno de estos campos estructurados al unir). La fusión NO usa `causa` como criterio, a propósito: descarga principal, listado y tercer nivel son tres superficies del portal que fallan de forma independiente, y agrupar sus reportes por `causa` le haría perder al consumidor la capacidad de saber cuál de las tres topó.

  **Los tramos del tercer nivel pueden traer documentos fuera del filtro pedido.** Con folios no contiguos, el grupo se pide igual por el RANGO entre sus extremos (lo económico en llamadas), así que el SII puede devolver un documento vecino del mismo día y tipo que el listado no mostró. Ese documento no se descarta: el XML es el `SetDTE` firmado tal como lo entrega el SII, y no se reescribe para sacar contenido —haría falta reconstruir la `Caratula` y la `Signature` del envío, que no son parte de ningún `<DTE>` individual—. Es un documento real de la misma empresa: respaldarlo de más no rompe nada, y `documentos` lo cuenta.

  **`ok:true` con `limitaciones` no vacías es un respaldo PARCIAL, no un error.** `tramos` trae lo que SÍ se pudo bajar (con su `documentos` sumando sólo eso), y el consumidor tiene que volver a pedir cada sub-rango de `limitaciones` aparte —filtrando por `tipo_dte` si el motivo es de fecha, o repitiendo `tipo_dte`+`folio_desde`/`folio_hasta`+`contraparte_rut` si el motivo trae esos campos— en vez de reintentar el rango completo, que repetiría contra el SII las descargas que ya habían salido bien. Antes de este cambio, un solo sub-rango sin fondo hacía que la ruta descartara TODO lo ya bajado; con volumen real eso llevaba a reintentar el mes entero varias veces (una por `tipo_dte`), el barrido que `ritmoSii.ts` documenta como causa de bloqueo del portal.

  **Cuando NO se bajó nada** —todos los sub-rangos toparon, o el único tramo pedido topó y no había hermanos— la ruta sigue respondiendo `LIMITE_CONOCIDO` **con `detalle`**, igual que antes: un `ok:true` con `tramos:[]` sería indistinguible de "el período no tuvo documentos". No es `ERROR` porque el fallo es permanente y el mensaje dice qué hacer; `ERROR` significa "reintentá", y reintentar el mismo rango no cambia nada acá. Si hubo **más de una** limitación, `detalle` las une con `' | '` (no con salto de línea), igual que el resto de los `detalle` de este servicio.
...
| Endpoint | Body | Devuelve |
|---|---|---|
| **`POST /v1/rcv/eventos-acuse`** | `rut` + credencial | catálogo de eventos válidos: ERM (recibo mercaderías/servicios), ERG (recibo guía mes anterior) |
| **`POST /v1/rcv/acuse`** | `documentos:[{rut_emisor,tipo_doc,folio}], evento, confirmar?` | `{ejecutado, evento, documentos, mensaje}` |

El **dry-run valida el evento contra el catálogo, pero NO verifica que los documentos existan** en el RCV (no cruza contra el detalle): confirma la forma del acto, no que el folio exista. Salvaguardas: el `evento` se valida contra el catálogo del SII antes de escribir; la respuesta 100 del SII (alerta) NO se reporta como éxito; hay una red anti-doble-click (el mismo acuse repetido en <60 s se rechaza). La traza de auditoría distingue `simulado`, `ejecutado` y `fallido` (un `confirmar:true` que no cursó) y guarda la referencia (evento + documentos, truncada) en la tabla `auditoria` (columnas `efecto`/`referencia`, migración 0002). **Esa traza sólo existe en el camino REST**: un acuse cursado por la tool MCP `sii_rcv_acusar` no deja fila en `auditoria` (el servidor MCP es stdio, sin Neon), así que para un acto auditado conviene usar REST.
...
| **`POST /v1/bhe/observar`** | `sii_bhe_observar` | `anio, mes, folio, causa (1\|2), confirmar?` |
| **`POST /v1/bhe/email`** | `sii_bhe_email` | `codigo_barras, email?, confirmar?` |
...
Observar (operación del RECEPTOR sobre una boleta RECIBIDA): `causa` 1 = no se pagó el servicio, 2 = no se prestó; `anio`/`mes` ubican la boleta en el informe de recibidas. Irreversible: la observación queda registrada y no se puede borrar. Verificada la cadena hasta la previsualización con una recibida real; el acto final replica el POST del portal (`TMBANU_RecepcionRespuestaRechazo`).

Email: reenvía la boleta EMITIDA por correo, identificada por su **código de barras** (no el folio). Sin `email`, va al que el portal tiene del receptor; la previsualización lo devuelve (`emailPortal`/`emailDestino`) para revisarlo antes de confirmar.

Con esto la familia BHE de escritura queda completa: emitir / anular / observar / email, todas con dry-run por default, red anti-doble-click y auditoría.
