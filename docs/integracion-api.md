...
- **`POST /v1/mipyme/respaldo-xml`** — el XML firmado de los DTE de un rango. `rut`, `fecha_desde` y `fecha_hasta` (`AAAA-MM-DD`) son obligatorios; opcionales `empresa_rut`, `origen` (`recibidos` por defecto, o `emitidos`), `tipo_dte`, `contraparte_rut`, `razon_social`, `folio_desde`, `folio_hasta` y `max_tramos` (default 10).
  ...
  **`causa`**: discriminador ESTRUCTURADO, siempre presente en la respuesta: `"PRESUPUESTO_TRAMOS"` cuando el corte fue por agotar `max_tramos` (las cuatro filas de la tabla marcadas así) o `"OTRA"` para el resto (incluye el caso interno sin clasificar; la ruta le pone `"OTRA"` cuando falta, así el campo nunca queda ausente en el JSON). Existe para no tener que parsear `motivo` en prosa cuando lo único que hace falta saber es "¿este respaldo parcial es por `max_tramos`, o por otra razón?" — que es justo la pregunta que decide si repetir la consulta con un `max_tramos` más alto tiene sentido. No reemplaza a `motivo`, que sigue siendo el texto completo con el detalle accionable.
