...
## Contrato: qué quedó implementado

> **Nota de cierre.** Este relevamiento se hizo en paralelo con otra sesión que
> implementó la misma ficha y llegó al mismo hallazgo por su cuenta. La ruta que
> quedó es la suya, **`POST /v1/misii/datos-contribuyente`** (PR #71), y sobre
> ella se portó lo que este relevamiento aportaba (PR #73). Lo de abajo era la
> propuesta original; se conserva porque su razonamiento es el que se aplicó.

`POST /v1/misii/datos-contribuyente`, body `{rut, clave}` (o certificado; desde
...
1. Parser sobre las variables embebidas, con fallo ruidoso si alguna falta.
   **Hecho** en los PR #71 y #73.
2. Ruta REST + tool MCP a la par. **Hecho**: `/v1/misii/datos-contribuyente` y
