...
| R11 (en curso) | Escritura de portal | — | **2** | 🚧 Arrancó por RCV acuse de recibo (`/v1/rcv/eventos-acuse`, `/v1/rcv/acuse`), que fija el guardrail: flag `confirmar` (dry-run por default), validación de evento contra catálogo, red anti-doble-click, traza de auditoría (efecto/referencia). Hecho: RCV acuse (#79) y mipyme guardar-borrador. Hecho: RCV acuse, mipyme borrador, BHE emitir. Faltan: BHE anular/observar/email (requieren una boleta real relevada). Fuera: set_tipo_transaccion (app legacy) y set_resumen (no existe). Diseño en 2026-08-31-ronda-11-escritura-design.md. |
