...
describe('registrarRutasMipyme', () => {
  ...
  describe('respaldo-xml', () => {
      ...
      expect(body2.limitaciones).toEqual([
        {
          ...
          // `causa` siempre presente en el wire: la ruta le pone
          // 'SIN_CLASIFICAR' cuando el scraper no clasificó la limitación
          // (distinto de 'OTRA', que significa "clasificado, y ninguna
          // acción particular sirve"), para que el campo nunca quede ausente
          // por la elisión de `undefined` en JSON.
    ...
    it('omite tipo_dte/contraparte_rut/razon_social/folio_desde/folio_hasta del JSON cuando el motivo no es del tercer nivel', async () => {
      ...
      // `causa` viaja siempre (con el default 'SIN_CLASIFICAR' cuando el
      // scraper no clasificó — distinto de 'OTRA'), a diferencia de los
      // campos del tercer nivel que sí se omiten del JSON cuando no aplican.
      expect(Object.keys(porHttp.limitaciones[0]).sort()).toEqual(['causa', 'fecha_desde', 'fecha_hasta', 'motivo']);
      expect(porHttp.limitaciones[0].causa).toBe('SIN_CLASIFICAR');
    });
